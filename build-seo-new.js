...
function parseCSV(t) {
  ...
  const paidTierValue = (s) => {
    const value = hkey(s);
    if (['hiring_boost', 'hiringboost', 'boost', 'boosted', 'paid_boost'].includes(value)) return 'hiring_boost';
...
function jobBranchNavHTML(job) {
  ...
  return `<nav class="seo-branch-nav" aria-label="Job page links" style="display:flex;flex-wrap:wrap;gap:10px;margin:0 auto 1rem;max-width:1120px;padding:1rem 1.5rem;font-size:14px;">
    <a href="/" style="color:var(--primary);font-weight:700;text-decoration:none;">MapleDevs home</a>
    <span style="color:var(--text-3);">/</span>
    <a href="/jobs/" style="color:var(--primary);font-weight:700;text-decoration:none;">All jobs</a>
    ${city ? `<span style="color:var(--text-3);">/</span><a href="${escapeHTML(boardFilterURL('city', city))}" style="color:var(--primary);font-weight:700;text-decoration:none;">${escapeHTML(city)} jobs</a>` : ''}
    ${studio ? `<span style="color:var(--text-3);">/</span><a href="${escapeHTML(boardFilterURL('q', studio))}" style="color:var(--primary);font-weight:700;text-decoration:none;">${escapeHTML(studio)} jobs</a>` : ''}
  </nav>`;
}
...
function injectSEO(html, target, targetJobs = []) {
  ...
  // 3. Add CSS and JS links
  const cssLinks = CSS_FILES.map(css => `<link rel="stylesheet" href="${css}">`).join('\n');
  const jsLink = `<script src="${JS_FILE}" defer></script>`;
  output = output.replace('</head>', `${cssLinks}\n${jsLink}\n</head>`);

  // 4. Redirect / Deep Link Hash (for SPA fallback)
  ...
  // 6. Job Branch Navigation for job pages
  ...
  if (HUB_CONTEXTS[target.folder]) {
    ...
    const ctxHtml = `<div class="hub-highlights" style="display:block; margin-bottom:1rem; border-left:4px solid #2563EB; padding:1rem; background:#EFF6FF;">
            <h3 style="margin:0 0 0.5rem 0; font-size:1.1rem; color:#2563EB;">${ctx.t}</h3>
...
async function build() {
  ...
  try {
    ...
    csvData = fs.readFileSync(path.join(ROOT_DIR, 'live.csv'), 'utf-8');
  }
  ...
  for (const target of SEO_TARGETS) {
    ...
    if (p.has('city')) targetJobs = jobs.filter(j => j.location.toLowerCase().includes(p.get('city').toLowerCase()));
      ...
      const ro = p.get('role');
      const r = ro==="programming"?/programmer|engineer|develop|tech|backend|frontend|c\+\+/i:ro==="art"?/art|animat|vfx|3d|2d|model/i:ro==="design"?/design|level|ux|ui|narrative/i:ro==="qa"?/qa|test|quality/i:ro==="production"?/produc|manage|coordinat/i:/.*/;
    ...
    const html = injectSEO(baseHTML, pageTarget, targetJobs.slice(0, 10));
  ...
  for (const job of jobs) {
    ...
    const html = injectSEO(baseHTML, target, [job]);
