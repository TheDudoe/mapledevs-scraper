...
const privacyIndex = content.indexOf(privacyMarker);
const mainScriptStartIndex = content.indexOf('<script>', content.indexOf(mainScriptMarker) - 100);
const mainScriptEndIndex = content.indexOf('</script>', mainScriptStartIndex);
const bannerIndex = content.indexOf(bannerMarker);
const bannerEndIndex = content.indexOf('</html>', bannerIndex) + 7;

if (privacyIndex === -1 || mainScriptStartIndex === -1 || mainScriptEndIndex === -1 || bannerIndex === -1) {
    console.error('Markers not found', { privacyIndex, mainScriptStartIndex, mainScriptEndIndex, bannerIndex });
    process.exit(1);
}

const headerAndPrePrivacy = content.substring(0, privacyIndex);
const fixedPrivacyTermsFooter = `
<!-- ═══════ PRIVACY VIEW ═══════ -->
...
const scriptSection = content.substring(mainScriptStartIndex, mainScriptEndIndex + 9);
const bannerAndClosing = content.substring(bannerIndex, bannerEndIndex);

const finalContent = headerAndPrePrivacy + fixedPrivacyTermsFooter + scriptSection + '\n\n' + bannerAndClosing;

fs.writeFileSync(path, finalContent);
console.log('Fixed duplication and structural issues perfectly.');
