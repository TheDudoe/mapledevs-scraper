...
const privacyStart = '<!-- ═══════ PRIVACY VIEW ═══════ -->';
const startIndex = content.indexOf(privacyStart);

if (startIndex === -1) {
    console.error('Could not find Privacy View marker');
    process.exit(1);
}

// We want to keep everything before the FIRST occurrence of Privacy View.
const healthyStart = content.substring(0, startIndex);

const fixedRest = `
<!-- ═══════ PRIVACY VIEW ═══════ -->
<div id="privacy-view" class="view" role="main"><div class="about-c">
...
// Now find the scripts. They were duplicated too, so let's find the original script block.
// The script block always starts with <script> and usually contains SHEET_DOC_ID.
const scriptMarker = '<script>';
const mainScriptMarker = 'const SHEET_DOC_ID =';
let scriptIndex = content.indexOf(mainScriptMarker);
// We want to find the <script> tag BEFORE this marker.
let realScriptStart = content.lastIndexOf('<script>', scriptIndex);

if (realScriptStart === -1) {
    console.error('Could not find real script start');
    process.exit(1);
}

// We want to keep everything from the real script start to the end of the file (one copy).
// But wait, the end of the file might have the duplicated banner.
const bannerMarker = '<!-- Sticky Feature Banner -->';
const lastBannerIndex = content.lastIndexOf(bannerMarker);
const closingTags = '</body>\n</html>';

const finalScripts = content.substring(realScriptStart, content.indexOf('</script>', lastBannerIndex) + 9) + '\n\n' + content.substring(lastBannerIndex);

fs.writeFileSync(path, healthyStart + fixedRest + finalScripts);
